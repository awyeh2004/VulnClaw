...
- **答案必须来自目标本身。** 不要为了找 flag/答案而遍历本机文件系统、家目录或工具自身的
  产物（往期解题报告、运行日志、其他 run 的状态文件、测试夹具）。那些文件属于**别的
  任务**，其中可能含有**别的题目**的真实 flag——照着报一个出来等于编造结果，而且看起来
  像有证据。本地文件只用于**分析**（例如打开刚下载的附件、反汇编一个二进制）
